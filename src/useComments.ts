...
import { api } from './api'
import { objectPointOnSlide, resolveCommentAnchor, slidePointToViewport } from './comment-geometry'
...
export function useComments(options: {
  ...
  const locationLabel = (thread: CommentThread) => {
    ...
    return detachmentLabel(resolved.detached) || (resolved.offSlide ? 'Object is outside the slide bounds' : thread.anchor.kind === 'object' ? `Object · ${editor.graph.getNode(thread.anchor.nodeId!)?.name ?? thread.nodeName}` : 'Point on slide')
  }
  ...
  async function reload() {
    ...
    loading.value = true
    try {
      const result = await api<CommentThread[]>(`/decks/${options.deckId}/comments`)
      if (destroyed || token !== requestGeneration) return
  ...
  async function startAt(anchor: CommentAnchorInput) {
    ...
    if (!editor.graph.getNode(anchor.slideId) || anchor.slideId !== active.value) { error.value = 'The target slide changed. Open its context menu again.'; return }
    if (anchor.kind === 'object') {
      if (!objectPointOnSlide(editor.graph, anchor.nodeId, anchor.slideId)) { error.value = 'The clicked object no longer belongs to this slide.'; return }
  ...
  async function startObject() {
    ...
    if (!nodeId || !objectPointOnSlide(editor.graph, nodeId, active.value)) { error.value = 'Select an object on the current slide first.'; return }
  ...
  async function post() {
    ...
    await mutation(async () => {
      await options.flush()
      return api(`/decks/${options.deckId}/comments`, { method: 'POST', body: JSON.stringify({ id: draftId, anchor, body, expectedRevision: options.revision.value }) })
    }, thread => { cancelDraft(); focusedId.value = thread.id })
  }
  async function reply() {
    const thread = focused.value
    if (!thread || !replyBody.value.trim()) return
    await mutation(() => api(`/decks/${options.deckId}/comments/${thread.id}/replies`, {
  ...
  async function setResolved(thread: CommentThread, resolved: boolean) {
    ...
    await mutation(() => api(`/decks/${options.deckId}/comments/${thread.id}`, {
  ...
  async function ask(thread: CommentThread) {
    ...
    if (attachment(thread).detached === 'slide_deleted') { error.value = 'The referenced slide was deleted. This thread stays available for reference.'; return }
    selectThread(thread)
    await options.handoff({
      commentThreadId: thread.id, expectedCommentVersion: thread.version,
      message: `Please address comment #${number(thread.id)} on "${slideTitle(thread)}":\n${thread.messages[0]!.body}`,
    })
  }
  ...
  return { threads, open, loading, loaded, saving, pointMode, showResolved, error, focusedId, focused, draftAnchor, draftBody, replyBody, hasDraft, visible, unresolvedCount, pins, draftPin, canAttach, number, attachment, locationLabel, slideTitle, reload, startPoint, placePoint, startObject, startAt, cancelDraft, cancelReply, selectThread, post, reply, setResolved, ask }
}
